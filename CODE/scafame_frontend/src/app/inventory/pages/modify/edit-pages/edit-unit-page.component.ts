// src/app/inventory/pages/modify/edit-pages/edit-unit-page.component.ts
import { Component } from '@angular/core';
import { EditarFormularioComponent } from '../../../../shared/components/forms/editar-formulario.component';
import { ButtonComponent } from '../../../../shared/components/button/button.component';
import { Router } from '@angular/router';

@Component({
  standalone: true,
  selector: 'app-edit-unit-page',
  imports: [EditarFormularioComponent, ButtonComponent],
  template: `
    <main class="pt-2 px-8 font-display min-h-[calc(100vh-120px)] flex items-start">
      <div class="max-w-[1200px] mx-auto w-full flex flex-col items-center justify-center relative">
        <h1 class="text-4xl font-bold text-black text-center mb-5">Editar Unidad</h1>
        <app-editar-formulario [tipo]="'Unidad'"></app-editar-formulario>
        <div class="flex justify-center mt-10 lg:absolute lg:right-0 lg:bottom-1 lg:items-end lg:mt-0">
          <app-button label="Volver" variant="light" (click)="goBack()" />
        </div>
      </div>
    </main>
  `
})
export class EditUnitPageComponent {
  constructor(private router: Router) {}
  goBack() { this.router.navigate(['/modify']); }
}
