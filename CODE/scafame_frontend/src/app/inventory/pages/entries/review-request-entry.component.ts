  ...
  template: `
    <main class="min-h-[calc(100vh-120px)] p-8 font-display">
      <div class="max-w-[1100px] w-full mx-auto relative flex flex-col justify-center">
        <h1 class="text-4xl font-bold text-black text-center mb-10">Realizar Ingreso</h1>
      ...
      <div *ngIf="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white text-black p-6 rounded-lg max-w-md w-full relative">
          <h2 class="text-xl font-bold mb-4">Confirmar Ingreso</h2>
...
export class ReviewEntryComponent implements OnInit {
  selected: { id: number; nombre: string; cantidad: number; unidad?: string }[] = [];
  showModal = false;
  ...
  ngOnInit(): void {
    this.selected = this.entryService.getSelectedProducts();
  }
    ...
    this.authService.getPerfilUsuario().subscribe({
      next: (user) => {
        const payload = {
          type: 'income',
          userId: user.id,
          status: 'APPROVED', // <- Forzado desde frontend
        ...
        this.reportService.createIncomeReport(payload).subscribe({
          next: () => {
